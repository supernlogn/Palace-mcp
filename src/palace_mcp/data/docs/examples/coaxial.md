# [Signal Propagation in a Coaxial Cable](#Signal-Propagation-in-a-Coaxial-Cable)[]()[](#Signal-Propagation-in-a-Coaxial-Cable)

The files for this example can be found in the [`examples/coaxial/`](https://github.com/awslabs/palace/blob/v0.16.0/examples/coaxial) directory of the *Palace* source code.
...
In this example we consider three different configurations of the model, all with a coaxial lumped port excitation at one end of the line: an open termination at the opposite end ([`coaxial_open.json`](https://github.com/awslabs/palace/blob/v0.16.0/examples/coaxial/coaxial_open.json)), a shorted termination ([`coaxial_short.json`](https://github.com/awslabs/palace/blob/v0.16.0/examples/coaxial/coaxial_short.json)), and a matched $50\text{ }\Omega$ lumped port termination ([`coaxial_matched.json`](https://github.com/awslabs/palace/blob/v0.16.0/examples/coaxial/coaxial_matched.json)).

The mesh is generated using the Julia code in [`mesh/mesh.jl`](https://github.com/awslabs/palace/blob/v0.16.0/examples/coaxial/mesh/mesh.jl) and consists of quadratically-curved hexahedral elements, as depicted below. Third-order shape functions are used to approximate the solution.
...
The excitation pulse is configured under [`config["Solver"]["Transient"]`](../../config/solver/#solver%5B%22Transient%22%5D). Here, we use a modulated Gaussian pulse shape, with time dependence given by the expression
