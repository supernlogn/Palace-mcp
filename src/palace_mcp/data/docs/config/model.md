...
`"Mesh" [None]` :  Input mesh file path, an absolute path is recommended. If the provided mesh is nonconformal, it is assumed that it comes from a previous *Palace* solve using AMR, and all mesh preprocessing checks and modifications (for example [`model["Refinement"]["CrackInternalBoundaryElements"]`](#model%5B%22Refinement%22%5D)), are skipped .
...
## [`model["Refinement"]`](#model["Refinement"])[]()[](#model["Refinement"])
...
`"Tol" [1.0e-2]` : Stop adaptive mesh refinement (AMR) when the norm of the estimated error is below this value. The error is reported in `error-indicators.csv`.
...
`"Nonconformal" [true]` : Choose whether the adaptation should use nonconformal refinement. Nonconformal refinement is required for non-simplex meshes.
...
### [Advanced model options](#Advanced-model-options)[]()[](#Advanced-model-options)
