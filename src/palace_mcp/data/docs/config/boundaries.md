# [`config["Boundaries"]`](#config["Boundaries"])[]()[](#config["Boundaries"])
...
`"WavePortPEC"` :  Top-level object for configuring additional PEC boundary conditions for boundary mode analysis performed on the wave port boundaries. Thus, this object is only relevant when wave port boundaries are specified under [`config["Boundaries"]["WavePort"]`](#boundaries%5B%22WavePort%22%5D).
...
## [`boundaries["PEC"]`](#boundaries["PEC"])[]()[](#boundaries["PEC"])
...
## [`boundaries["PMC"]`](#boundaries["PMC"])[]()[](#boundaries["PMC"])
...
## [`boundaries["Impedance"]`](#boundaries["Impedance"])[]()[](#boundaries["Impedance"])
...
## [`boundaries["Absorbing"]`](#boundaries["Absorbing"])[]()[](#boundaries["Absorbing"])
...
## [`boundaries["Conductivity"]`](#boundaries["Conductivity"])[]()[](#boundaries["Conductivity"])
...
## [`boundaries["LumpedPort"]`](#boundaries["LumpedPort"])[]()[](#boundaries["LumpedPort"])
...
`"Excitation" [false/0]` :  Turns on or off port excitation for this lumped port boundary for driven or transient simulation types. Can be specified either as a bool or as a non-negative integer — see [Boundary Conditions](../../guide/boundaries/#Lumped-and-wave-port-excitation).
...
## [`boundaries["WavePort"]`](#boundaries["WavePort"])[]()[](#boundaries["WavePort"])
...
`"Excitation" [false/0]` :  Turns on or off port excitation for this wave port boundary for driven simulation types. Can be specified either as a bool or as a non-negative integer — see [Boundary Conditions](../../guide/boundaries/#Lumped-and-wave-port-excitation).
...
`"SolverType" ["Default"]` :  Specifies the eigenvalue solver to be used in computing the boundary mode for this wave port. See [`config["Solver"]["Eigenmode"]["Type"]`](../solver/#solver%5B%22Eigenmode%22%5D).
...
## [`boundaries["WavePortPEC"]`](#boundaries["WavePortPEC"])[]()[](#boundaries["WavePortPEC"])
...
`"Attributes" [None]` :  Integer array of mesh boundary attributes to consider as PEC when solving the 2D eigenproblem for the wave port boundary mode analysis, along with those specified under [`config["Boundaries"]["PEC"]["Attributes"]`](#boundaries%5B%22PEC%22%5D) and [`config["Boundaries"]["Conductivity"]["Attributes"]`](#boundaries%5B%22Conductivity%22%5D).

## [`boundaries["SurfaceCurrent"]`](#boundaries["SurfaceCurrent"])[]()[](#boundaries["SurfaceCurrent"])
...
`"Direction" [None]` :  Defines the source current direction for this surface current boundary. The available options are the same as under [`config["Boundaries"]["LumpedPort"]["Direction"]`](#boundaries%5B%22LumpedPort%22%5D). If this source is to be a multielement source which distributes the source across more than a single lumped element, use the `"Elements"` array described below.

`"CoordinateSystem" ["Cartesian"]` :  Defines the coordinate system for the source current direction for this surface current boundary. The available options are the same as under [`config["Boundaries"]["LumpedPort"]["CoordinateSystem"]`](#boundaries%5B%22LumpedPort%22%5D). If this source is to be a multielement source which distributes the source across more than a single lumped element, use the `"Elements"` array described below.
...
## [`boundaries["Ground"]`](#boundaries["Ground"])[]()[](#boundaries["Ground"])
...
## [`boundaries["ZeroCharge"]`](#boundaries["ZeroCharge"])[]()[](#boundaries["ZeroCharge"])
...
## [`boundaries["Terminal"]`](#boundaries["Terminal"])[]()[](#boundaries["Terminal"])
...
## [`boundaries["Periodic"]`](#boundaries["Periodic"])[]()[](#boundaries["Periodic"])
...
`"FloquetWaveVector" [None]` :  Optional floating point array defining the phase delay between the periodic boundaries in the X/Y/Z directions in radians per mesh unit.

## [`boundaries["Postprocessing"]["SurfaceFlux"]`](#boundaries["Postprocessing"]["SurfaceFlux"])[]()[](#boundaries["Postprocessing"]["SurfaceFlux"])
...
## [`boundaries["Postprocessing"]["Dielectric"]`](#boundaries["Postprocessing"]["Dielectric"])[]()[](#boundaries["Postprocessing"]["Dielectric"])
...
`"Type" [None]` :  Specifies the type of dielectric interface for this postprocessing boundary. See also [this page](../../reference/#Bulk-and-interface-dielectric-loss). Available options are:
...
## [`boundaries["Postprocessing"]["FarField"]`](#boundaries["Postprocessing"]["FarField"])[]()[](#boundaries["Postprocessing"]["FarField"])
