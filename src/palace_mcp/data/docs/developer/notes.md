...
First, the mesh comes with its own unit of length. Users specify the conversion between one unit of length on the mesh to meters through the [`config["Model"]["L0"]`](../../config/problem/#config%5B%Model%22%5D) parameter.
...
### [Non-dimensional unit system](#Non-dimensional-unit-system)[]()[](#Non-dimensional-unit-system)

The non-dimensional unit system is constructed as follows:

Length: Lengths are measured in units of the characteristic length `Lc`, which is the mesh size in mesh units, unless `Lc` is manually specified in [`config["Model"]["Lc"]`](../../config/problem/#config%5B%Model%22%5D). The extent of the mesh is 1 in these units, with extent defined as the largest single dimension (e.g., for a cuboid of size 10m x 20m x 40m, this would be 40m).
...
Electromagnetic quantities: *Palace* uses [Heaviside-Lorentz](https://en.wikipedia.org/wiki/Heaviside%E2%80%93Lorentz_units) units, essentially meaning that `ε₀ = μ₀ = 1` (so that in addition to setting `c = 1`, the impedance of free space `Z₀` is also 1).
...
Consider a spherical mesh with `L0` of 1 micron and radius of 3 in mesh units. Then:
...
Automated source code formatting is performed using [`clang-format`](https://clang.llvm.org/docs/ClangFormat.html). Run:

```
./scripts/format_source
```

in the repository root directory to automatically use `clang-format` to format `C++` source as well as [`JuliaFormatter.jl`](https://github.com/domluna/JuliaFormatter.jl) for Julia and Markdown files. The script can be viewed [in the repository](https://github.com/awslabs/palace/blob/v0.16.0/scripts/format_source).

The following conventions also apply:

- `PascalCase` for classes and function names.
- Follow 'include what you use' (IWYU), with the include order dictated by the [Google C++ Style Guide](https://google.github.io/styleguide/cppguide.html#Names_and_Order_of_Includes). This order should be automatically enforced by the `clang-format` [style file](https://github.com/awslabs/palace/blob/v0.16.0/.clang-format).
...
During the `cmake` configuration step, defining the variables `ANALYZE_SOURCES_CLANG_TIDY` and `ANALYZE_SOURCES_CPPCHECK` to `ON` will turn on static analysis using [`clang-tidy`](https://clang.llvm.org/extra/clang-tidy/) and [`cppcheck`](https://cppcheck.sourceforge.io/), respectively, during the build step. This requires the executables to be installed and findable by CMake on your system.

## [Extending GPU code in *Palace*](#Extending-GPU-code-in-*Palace*)[]()[](#Extending-GPU-code-in-*Palace*)

*Palace* supports [GPU parallelization](../../guide/parallelism/), but not all the files in *Palace* are compiled with a GPU-compatible compiler (e.g., `nvcc`). The reason for this is that such compilers might not support all the features *Palace* needs (e.g., support for `constexpr std::sqrt`), and also because they tend to be slower. The list of files that contains code that has to run on the device is defined by the `TARGET_SOURCES_DEVICE` CMake variable.

## [JSON Schema for configuration files](#JSON-Schema-for-configuration-files)[]()[](#JSON-Schema-for-configuration-files)

A JSON format [configuration file](../../config/config/), for example named `config.json`, can be validated against the provided Schema using:

```
./scripts/validate_config config.json
```

[This script](https://github.com/awslabs/palace/blob/v0.16.0/scripts/validate_config) uses Julia's [`JSONSchema.jl`](https://github.com/fredo-dedup/JSONSchema.jl) and the Schema provided in [`scripts/schema/`](https://github.com/awslabs/palace/blob/v0.16.0/scripts/schema) to parse the configuration file and check that the fields are correctly specified. This script and the associated Schema are also installed and can be accessed in `<INSTALL_DIR>/bin`.
...
## [Profiling *Palace* on CPUs](#Profiling-*Palace*-on-CPUs)[]()[](#Profiling-*Palace*-on-CPUs)
...
### [Building *Palace* with VTune Support](#Building-*Palace*-with-VTune-Support)[]()[](#Building-*Palace*-with-VTune-Support)
...
### [Running a Basic Profile](#Running-a-Basic-Profile)[]()[](#Running-a-Basic-Profile)
...
Below is a sample flamegraph resulted from profiling the rings example located at [`/examples/rings/rings.json`](https://github.com/awslabs/palace/blob/v0.16.0/examples/rings/rings.json) with polynomial order P=6 (`Solver["Order"]: 6`).
...
## [Developing the spack recipe](#Developing-the-spack-recipe)[]()[](#Developing-the-spack-recipe)
...
will show you the spec for your most recent `palace` package, which you can compare with the one for `builtin.palace@develop`.

## [Changelog](#Changelog)[]()[](#Changelog)

Code contributions should generally be accompanied by an entry in the [changelog](https://github.com/awslabs/palace/blob/v0.16.0/CHANGELOG.md).
