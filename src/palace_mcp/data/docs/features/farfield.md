# [Extracting Fields in the Radiation Zone](#Extracting-Fields-in-the-Radiation-Zone)[]()[](#Extracting-Fields-in-the-Radiation-Zone)

When run in the [`Driven` or `Eigenmode` modes](../../guide/problem/), *Palace* can extrapolate fields from the near-field region (being simulated) to the far-field zone. This capability can be used to study the radiative properties of a system.

The mathematical details on how this is accomplished are available in the [reference](../../reference/#Far-field-extraction). The key points are as follows:
...
- For the result to be physically accurate, it is important to properly model the propagation of waves to infinity. This can be accomplished by enclosing the system inside a sphere or box and applying [`config["Boundaries"]["Absorbing"]` boundary conditions](../../config/boundaries/#boundaries%5B%22Absorbing%22%5D).
- The result is provided as complex vectors $r \mathbf{E}(\theta, \phi)$, where $(\theta, \phi)$ identify a point on sphere at infinite distance and the result is defined up to a global phase.

*Palace* outputs $r\mathbf{E}$ because this is a well-defined, finite quantity ($\mathbf{E}$ itself goes to zero at infinity as 1/r). You can also think of this as the electric field measured at a distance of one unit of length. With this output, you can immediately compute various quantities of interest. For instance, $|r\mathbf{E}|^2$ gives the relative radiative power.

*Palace* only supports propagation of fields to infinity when the integration surface
...
Both `"NSample"` and `"ThetaPhis"` can be specified simultaneously and the results will be combined and duplicates removed.

## [Output](#Output)[]()[](#Output)

Once a simulation is run, *Palace* generates a CSV file named `farfield-rE.csv` in the folder specified by the [`config["Problem"]["Output"]`](../../config/problem/#config%5B%22Problem%22%5D) configuration.
...
This type of output can be processed by several different packages and languages. For instance, see the [antenna example](../../examples/antenna/) for an example of a [Julia](https://julialang.org/) script that plots antenna patterns.
