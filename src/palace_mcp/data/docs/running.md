# [Running *Palace*](#Running-*Palace*)[]()[](#Running-*Palace*)
...
- The installed [`palace`](https://github.com/awslabs/palace/blob/v0.16.0/scripts/palace) script wraps a call to the desired MPI launcher (`mpirun` by default).
- `<NUM_PROCS>` is the number of MPI processes to use for the simulation.
- `config.json` is the JSON format configuration file used to specify the simulation parameters. The structure of this configuration file is outlined in detail in the section [Configuration File](../config/config/).
...
## [Singularity/Apptainer](#Singularity/Apptainer)[]()[](#Singularity/Apptainer)
